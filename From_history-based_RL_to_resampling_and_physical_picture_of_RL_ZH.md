...
对多维变量 $\boldsymbol{x}$，若协方差矩阵为 $\Sigma$，则第 $k$ 轮的退火 proposal 为
```math
...
其中 $`c'_{a}`$ 表示扰动后的动作前缀，可以是 token-prefix、hidden-prefix、tool-call prefix 或 agent-step prefix。 $`o'_{\mathrm{allow}}`$ 表示**允许扰动**的观测字段。$`r'_{\mathrm{soft}}`$ 表示 soft score、verifier score、reward model score 等可以扰动的回报或评分。若只扰动动作，则固定 $`o'_{\mathrm{allow}}=o_{\mathrm{allow}}`$、 $`r'_{\mathrm{soft}}=r_{\mathrm{soft}}`$ 。若做联合扰动，则三者一起采样。

在 LLM 自回归情形下，动作前缀可以写成 $`c_{a,i,t} = (h_t,a_{L_t},a_{L_t+1},\cdots,a_i)`$，扰动后得到 $`c'_{a,i,t} \sim q_a(c'_{a,i,t}\mid c_{a,i,t})`$ 。允许扰动的观测量和回报可以写成 mask 形式
```math
\begin{aligned}
